...
import { NarrativeForm } from '@/components/qap/NarrativeForm'
...
export default async function NarrativePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fields = await db
    .select()
    .from(qapFields)
    .where(and(eq(qapFields.deal_id, deal.id), eq(qapFields.section, 'narrative')))

  const fieldMap = Object.fromEntries(fields.map(f => [f.field_key, f.value ?? '']))
