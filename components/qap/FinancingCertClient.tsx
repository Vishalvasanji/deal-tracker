...
import {
  ...
  USE_ROWS, CASH_SOURCE_ROWS, FEES_GRANTS_ROWS, type FinCertPulled,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">15. Sources of Cash:</p>
        {CASH_SOURCE_ROWS.map(r => (
          <div key={r.key} className="flex items-center justify-between gap-3">
            <span className="text-sm text-muted-foreground flex-1">{r.label}</span>
            <div className="w-40">{cell(r.key)}</div>
          </div>
        ))}
        <p className="text-xs text-muted-foreground pt-1">16. Source of Fees and Grants:</p>
        {FEES_GRANTS_ROWS.map(r => (
          <div key={r.key} className="flex items-center justify-between gap-3">
            <span className="text-sm text-muted-foreground flex-1">{r.label}</span>
            <div className="w-40">{cell(r.key)}</div>
          </div>
        ...
        <div className="flex items-center justify-between rounded-xl border border-border bg-muted/30 px-3 py-2">
          <span className="text-sm font-semibold">17. Total Cash, Fees &amp; Grants</span>
          <span className="text-sm font-bold tabular-nums">{money(cashSub + feesSub)}</span>
        </div>
      </div>
